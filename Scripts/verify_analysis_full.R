...
exact_permanova <- function(X, method = "bray", site = NULL) {
  ...
  if (!is.null(site)) stopifnot(identical(as.character(site), grp0))
...
main <- exact_permanova(b6$X, "bray", site = b6$site)
...
loco <- sapply(E6, function(drop) {
  ...
  exact_permanova(bb$X, "bray", site = bb$site)
})
...
n <- nrow(Db)
## This script does not call adonis2 for the partial model -- the exact
## p-value is computed directly via a McArdle-Anderson sequential
## sum-of-squares decomposition over all 512 restricted arrangements. See
## verify_reply_numbers.R Section M for a direct vegan::adonis2 cross-check
## of the corresponding F-statistic on this same design.
...
aitch_permanova <- function(clr_mat, site = NULL) {
...
obsA <- aitch_permanova(clr, site = b6$site)
...
bB <- build_prop(classes = ALL10, renorm = FALSE)
rB <- exact_permanova(bB$X, "bray", site = bB$site)
ok("B: 9 pairs, 10 classes, F", rB["F"], 6.95, tol = 0.05)
ok("B: R2", rB["R2"], 0.303, tol = 0.005)

bC <- build_prop(classes = E7)
rC <- exact_permanova(bC$X, "bray", site = bC$site)
...
if (n_fail > 0) {
  ...
  cat("      not something to quietly patch. Fix the reported number, not\n")
  cat("      the check.\n")
} else {
