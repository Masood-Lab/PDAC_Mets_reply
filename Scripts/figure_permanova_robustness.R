...
## Rung A ("unpaired, all 10 classes") comes from a different permutation
## scheme entirely -- 999 randomly sampled permutations (adonis2 default),
## not the paired exact-enumeration used for every other row. Its own floor
## is 1/1000 = 0.001, not 2/512 = 0.0039, so it is not comparable to the
## "at permutation floor" category on the P-value scale alone and would be
## miscategorized by a straight P <= 0.0039 threshold (0.002 < 0.0039).
## Flagged explicitly rather than left to the threshold check.
results$at_floor <- ifelse(
  results$label == "A: unpaired, all 10 classes", "different scheme (999 sampled perms)",
  ifelse(results$P <= 0.0039 + 1e-9, "at permutation floor",
...
p <- ggplot(results, aes(x = label, y = F, fill = at_floor)) +
  ...
  scale_fill_manual(values = c("at permutation floor" = "#1E7B34",
                                ...
                                "different scheme (999 sampled perms)" = "#999999")) +
