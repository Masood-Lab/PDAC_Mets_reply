...
build_design <- function(keep, src = keep, pats = PATS, d = spot) {
  ...
  X <- X / rowSums(X)
  ## The covariate must use the SAME six-habitat denominator as X.
  ## Averaging tumour_frac over ALL spots pulls in normal liver (CC6/CC8),
  ## which is 96.6% of PT_8's hepatic section, and gives F=7.61 / R2=0.240 /
  ## tf-alone R2=0.286 / Aitchison 9.05 -- none of which match. Restricting
  ## to `keep` reproduces the reply text exactly: 6.83 / 0.157 / 0.498 / 11.20.
...
if (fails == 0) {
  ...
  cat("  Any check that fails here means the reply text needs a correction.\n")
}
cat("\n  Not covered by this script, and not checked by any script in this\n")
cat("  repository:\n")
...
cat("  Those are the only figures in the reply text not checked here.\n\n")
