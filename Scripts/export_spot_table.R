...
## Path to the Seurat object is not committed to this repository (too large,
## and not needed by anyone downstream of the CSV this script produces).
## Pass it as a command-line argument:
##   Rscript export_spot_table.R /path/to/Pdac_allres_most_updated_V2.rds
## or set it directly below if running interactively.
args <- commandArgs(trailingOnly = TRUE)
rds_path <- if (length(args) >= 1) args[1] else "Pdac_allres_most_updated_V2.rds"
if (!file.exists(rds_path)) {
  stop("Seurat object not found at '", rds_path, "'.\n",
       "  This script is the one exception in this repository that requires\n",
       "  the original Seurat object rather than the deposited CSV -- pass its\n",
       "  path as the first argument, or edit rds_path above.")
}
pdac <- readRDS(rds_path)
...
stopifnot(all(c("Tumor Epithelial cells", "Hepatocytes", "myCAF") %in% rownames(rctd_mat)))
...
## myCAF_frac is read by two numbers in the reply (CC1/CC5 at 0.63/0.48
## myCAF, CC7 at 0.20 myCAF in the CC6->CC8->CC7 continuum) and by
## make_all_figures.py. treatment and iCAF_frac are not needed -- no
## script reads them and no reported number depends on them (PT_9 is
## excluded by patient ID in code, not via the treatment column).
